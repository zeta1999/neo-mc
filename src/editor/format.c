...
{
    edit_cursor_move (edit, q - edit->buffer.curs1, DATA_NULL);
    edit_delete (edit, TRUE, DATA_NULL);
    edit_insert_ahead (edit, c, DATA_NULL);
}
...
put_paragraph (WEdit * edit, unsigned char *t, off_t p, long indent, off_t size)
{
    ...
    for (i = 0; i < size; i++, p++)
    {
        if (i != 0 && indent != 0)
        {
            ...
            else if (t[i - 1] == '\n')
            {
                ...
                edit_cursor_move (edit, p - edit->buffer.curs1, DATA_NULL);
    ...
    edit_cursor_move (edit, cursor - edit->buffer.curs1, DATA_NULL);       /* restore cursor position */
}
