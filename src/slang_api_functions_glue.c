...
SLtype Multi_Type_Action_Data_ptr_Type = 0;

static int allocate_slang_api_functions_opaque_types(void) /* {{{ */
{
   if (Multi_Type_Action_Data_ptr_Type) return 0;
   ...
   Multi_Type_Action_Data_ptr_Type = allocate_opaque((char*)"Multi_Type_Action_Data_ptr_Type",
		(FINALIZER) NULL, 
		(INITIALIZER) NULL,
		(SLtype) void_ptr_Type, NULL, NULL);
   if (Multi_Type_Action_Data_ptr_Type == SLANG_UNDEFINED_TYPE) return -1;
...
static void sl_action (void)
{
   ...
   retval = slang_api__action(arg1, arg2,  arg3, arg4);
   (void)SLang_push_int(retval);
   goto free_and_return;
free_and_return:
   /* drop */
...
int init_slang_api_functions_module_ns(char *ns_name)	/* {{{ */
{
   ...
   if (allocate_slang_api_functions_opaque_types() == -1) return -1;
